...
def university(request, render_callback=None):
    # Get request context
    c = get_context(request, css_class='body-entities', title='Universidad')

    # Retrieve the entity to display
    university_entity = get_university_entity(c)
    set_title(c, university_entity.name)

    return entities_show(request, c, university_entity, render_callback)

def university_show(request, id, title, render_callback=None):
    # Get request context
...
def programmes_show_university(request, id, title, render_callback=None):
    # Get request context
    c = get_context(request, css_class='body-policies', title='')
    ...
    c['programme_id'] = id
    c['programme'] = FunctionalCategory.objects.filter( budget__entity=university_entity, 
                                                        programme=id)[0]
    c['policy'] = FunctionalCategory.objects.filter(budget__entity=university_entity, 
    ...
    c['funding_breakdown'] = BudgetBreakdown(['source', 'fund'])
    c['institutional_breakdown'] = BudgetBreakdown([_year_tagged_institution, _year_tagged_department])
    get_budget_breakdown(   "fc.programme = %s and e.id = %s", [ id, university_entity.id ],
                            [ 
                                c['economic_breakdown'],
                                c['funding_breakdown'],
                                c['institutional_breakdown']
    ...
    c['descriptions'] = Budget.objects.get_all_descriptions(university_entity).copy()
...
def income_articles_show_university(request, id, title, render_callback=None):
    return articles_show_university(request, id, title, 'income', render_callback)


def expense_articles_show_university(request, id, title, render_callback=None):
    return articles_show_university(request, id, title, 'expense', render_callback)


def articles_show_university(request, id, title, show_side, render_callback=None):
    # Get request context
    c = get_context(request, css_class='body-policies', title='')
    ...
    # Extra request context info
    c['article_id'] = id
    c['article'] = EconomicCategory.objects.filter( budget__entity=university_entity,
                                                    article=id, 
