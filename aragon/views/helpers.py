...
from aragon.models import Budget, BudgetItem, InflationStat, PopulationStat, Entity
...
def populate_stats(c):  # Convenience: assume it's top level entity
    populate_entity_stats(c, Entity.objects.region())
...
def populate_descriptions(c):
    c['descriptions'] = Budget.objects.get_all_descriptions(Entity.objects.region())

def populate_entity_descriptions(c, entity):
    c['descriptions'] = Budget.objects.get_descriptions(entity)
...
def populate_latest_budget(c):
    c['latest_budget'] = Budget.objects.latest(Entity.objects.region().id)

def populate_level(c, level):
    c['level'] = level
    c['is_county'] = (level=='comarca')
    c['show_entity_url'] = 'aragon.views.counties_show' if (level=='comarca') else 'aragon.views.towns_show'
