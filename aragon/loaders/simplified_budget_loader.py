...
class SimplifiedBudgetLoader:
    ...
    def parse_budget_data(self, budget_items, level, filename):
        ...
        for index, line in enumerate(reader):
            ...
            budget_id = SimplifiedBudgetLoader.BudgetId(line[3], year)
    ...
    def parse_non_xbrl_data(self, budget_items, level, filename):
        ...
        for index, line in enumerate(reader):
            ...
            budget_id = SimplifiedBudgetLoader.BudgetId(entity_id, year)
            if not budget_id in budget_items:
                budget_items[budget_id] = []
    ...
    def load_budget(self, level, path, entity_id, year, items):
        ...
        entity = Entity.objects.filter(level=level, code=entity_id)[0]
        if not entity:
        ...
        for item in items:
            ...
            if dimension == 'Functional' and chapter == 'P':
                ...
                chapter = SimplifiedBudgetLoader.functional_areas.get(description.lower().replace(' ',''),'')
                if chapter=='':
                    print u"ALERTA: No se encuentra el area funcional para '%s': %s€" % (description.decode("utf8"), amount)
                    continue

            uid = SimplifiedBudgetLoader.Uid(dimension, is_expense, is_actual, chapter, article, concept, subconcept)
    ...
    def keep_budget_item(self, items, uid, description, amount):
        ...
        # Add the item
        items[uid] = SimplifiedBudgetLoader.Item(description, amount)

        # Remove parent data, since the input data contains subtotals *sigh*
        if uid.subconcept != '':
            uid = SimplifiedBudgetLoader.Uid(uid.dimension, uid.is_expense, uid.is_actual, uid.chapter, uid.article, uid.concept, '')
            if uid in items:
                newAmount = SimplifiedBudgetLoader.Item(items[uid].description, items[uid].amount-amount)
            ...
            if uid.concept != '':
                uid = SimplifiedBudgetLoader.Uid(uid.dimension, uid.is_expense, uid.is_actual, uid.chapter, uid.article, '', '')
                if uid in items:
                    newAmount = SimplifiedBudgetLoader.Item(items[uid].description, items[uid].amount-amount)
                    if newAmount.amount < 0:
                        print "  Info: cantidad negativa '%s': %s" % (newAmount.description, newAmount.amount/100)
                ...
                if uid.article != '':
                    uid = SimplifiedBudgetLoader.Uid(uid.dimension, uid.is_expense, uid.is_actual, uid.chapter, '', '', '')
                    if uid in items:
                        newAmount = SimplifiedBudgetLoader.Item(items[uid].description, items[uid].amount-amount)
                        if newAmount.amount < 0:
                            print "  Info: cantidad negativa '%s': %s" % (newAmount.description, newAmount.amount/100)
    ...
    def load_functional_classification(self, path, budget):
        ...
        for index, line in enumerate(reader):
            ...
            # manually and need to jump through some extra hoops.
            SimplifiedBudgetLoader.functional_areas[line[3].lower()] = area
